...
export default function ExportDropdown({
  ...
  }, [activeTab, spItems, products, projectName, exporting, zh]);
  ...
  const handleExportPpt = useCallback(async () => {
    ...
    try {
      ...
      // SP slide
      if (spItems.filter(i => i.tier >= 1).length > 0) {
        const spSlide = pres.addSlide();
        spSlide.addText(zh ? '卖点分级' : 'SP Tier', { x: 0.5, y: 0.3, w: 9, fontSize: 24, bold: true, color: '1e2a3a' });
        let yPos = 1.0;
        for (const tier of [1, 2, 3]) {
          const tierItems = spItems.filter(i => i.tier === tier);
          if (tierItems.length === 0) continue;
          spSlide.addText(`T${tier}`, { x: 0.5, y: yPos, w: 1, fontSize: 16, bold: true, color: tier === 1 ? 'EF4444' : tier === 2 ? 'F59E0B' : '22C55E' });
          const itemTexts = tierItems.map(i => `• ${i.featureName}${i.paramValue ? `: ${i.paramValue}` : ''}`).join('\n');
          spSlide.addText(itemTexts, { x: 1.5, y: yPos, w: 8, fontSize: 14, color: '475569', lineSpacing: 22 });
  ...
  }, [spItems, projectName, segment, exporting, zh]);
