...
术语定义（Tier 含义、三层结构、字数风格、JSON 格式）见 <规则> 与 <示例>。
...
<规则>
## 三层结构
- **L1 营销名**:产品/卖点的命名层，是"参数+营销名"或纯参数。出现在官网卡片标题、电商详情页 hero 区
- **L2 Slogan**:一句话讲清卖点和用户利益点。是核心传播文案，作为主口号出现在官网、电商产品页、营销活动、发布会 PPT
- **L3 子卖点**:从使用场景、参数提升幅度、材质特点等角度拆解该卖点的具体利益点。出现在官网详情页、电商详情页、发布会 PPT

## 卖点分级（T1/T2/T3）

每条卖点行尾的 tier 标签决定 L1 命名风格与 L3 手法选择：

- **T1（参数领先）**:该价位段具有明显优势或用户关注度高的核心卖点
  - L1:推荐"参数+营销名"组合，营销名要酷炫有记忆点
  - L3:优先使用 equivalent(对比换算)、extreme(极限场景)手法
  - 是允许出现极限词的前置条件（仍需 hint 显式授权）

- **T2（参数持平）**:卖点参数与竞品接近，或用户关注度一般
