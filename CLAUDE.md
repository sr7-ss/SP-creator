...
面向手机品牌产品经理和营销人员的 AI 工具，帮助用户完成从市场调研 → 竞品参数对比 → KSP（Key Selling Point）分级 → 卖点包装全流程。
...
3. KSP 分级（Tier 1/2/3） → AI + 规则引擎自动划分卖点层级
...
| Review Mining | 批量分析用户评论，提取主题/情绪，建议 KSP 调整 | `src/lib/ai/agents/review-mining.ts` |
...
- **User Prompt**（动态）：KSP 条目 + 知识库匹配 + 调研上下文 + 参考项目风格
...
只注入和当前 KSP 条目 feature 匹配的条目，不做向量检索。
...
│   │   ├── projects/[id]/        # 项目 CRUD + products + ksp-results
│   │   ├── research/             # 调研报告存储
│   │   ├── knowledge/            # 知识库
│   │   ├── markets/              # 市场管理
│   │   └── user/                 # AI Key 管理 + 用量统计
│   ├── projects/[id]/page.tsx    # 主工作台（参数表 + KSP 看板 + 包装编辑器）
...
│   ├── analysis/
│   │   ├── rule-engine.ts        # 规则引擎（确定性 KSP 分级）
...
| KspResult | KSP 条目（tier 1-3, L1/L2/L3 包装文案） |
| Analysis | 竞品分析结果 |
...
| 模块 | 职责 |
|------|------|
| `src/lib/constants/slogan-rules.ts` | `SLOGAN_GENERATION_RULES` / `L3_PACKAGING_TECHNIQUES` / `KSP_TIER_RULES` |
...
| `/projects/[id]` | 主工作台（参数表 + KSP 看板 + 包装编辑器 + 导出） |
...
- **L3 存 `KspResult.l3Details`（JSON）而非独立表** — L3 永远跟着 KSP 走，不独立查询
